...
export function registerErrorHandler(app: FastifyInstance): void {
  app.setErrorHandler((err, _req, reply) => {
    ...
    if (statusCode === 413) return reply.status(413).send({ error: 'FILE_TOO_LARGE' });
    if (statusCode === 400) return reply.status(422).send({ error: 'INVALID_PARAMS', message: (err as Error).message });
    app.log.error(err);
